...
app.post('/send_tracking_request', (req, res) => {
  ...
  const from = parseInt(from_user_id);
  const to = parseInt(to_user_id);

  if (isNaN(from) || isNaN(to)) {
    return res.status(400).json({ error: 'Invalid user ID types. Expected integers.' });
  }
  ...
  db.query(sql, [from, to, 0], (err, result) => {
    if (err) {
      console.error("🔥 Database error in /send_tracking_request:", err?.sqlMessage || err.message || err);
      return res.status(500).json({ error: 'Database error.', details: err.message });
    }
...
app.get('/check_tracking_requests', (req, res) => {
  ...
  // Return all requests where to_user_id = user_id (status 0 = pending, 1 = accepted)
  const sql ='SELECT Log_ID, from_user_id, to_user_id, Status FROM user_request_tracking WHERE to_user_id = ? AND (Status = 0 OR Status = 1);'
...
app.post('/send_location', (req, res) => {
  const { from_user_id, to_user_id, latitude, longitude } = req.body;

  if (!from_user_id || !to_user_id || !latitude || !longitude) {
    return res.status(400).json({ error: 'Missing location data.' });
  }

  const sql = 'INSERT INTO user_location_tracking (from_user_id, to_user_id, latitude, longitude, timestamp) VALUES (?, ?, ?, ?, NOW())';

  db.query(sql, [from_user_id, to_user_id, latitude, longitude], (err, result) => {
    if (err) {
...
app.get('/get_latest_location', (req, res) => {
  const { user_id } = req.query;
  ...
  const sql = 'SELECT from_user_id, to_user_id, latitude, longitude, timestamp FROM user_location_tracking WHERE to_user_id = ? ORDER BY timestamp DESC LIMIT 1;'
